...
public class inMoney {
    ...
    StackPane main;
    Label tittle = new Label("الايرادات");
    ...
    Button delete;
    Button clear;
    public static TableView<inmoney> table;
    ...
    public inMoney() {
        ...
        table = new TableView();
        table.setItems(database.buildInMoneyTable("select * from inmoney"));
        table.setPrefSize(500.0D, 500.0D);
        TableColumn<inmoney, Integer> idColumn = new TableColumn("الكود");
        idColumn.setMinWidth(20.0D);
        idColumn.setCellValueFactory(new PropertyValueFactory("in_id"));
        ...
        TableColumn<inmoney, String> dateCoulmn = new TableColumn("التاريخ");
        dateCoulmn.setMinWidth(30.0D);
        dateCoulmn.setCellValueFactory(new PropertyValueFactory("in_date"));
        table.getColumns().addAll(new TableColumn[]{idColumn, priceCoulmn, notesCoulmn, dateCoulmn});
        table.setOnMouseClicked((event) -> {
            try {
                
                id.setText(((inmoney) table.getSelectionModel().getSelectedItem()).getIn_id() + "");
                notes.setText(((inmoney) table.getSelectionModel().getSelectedItem()).getIn_notes());
                price.setText(((inmoney) table.getSelectionModel().getSelectedItem()).getIn_price());
                date.setValue(LOCAL_DATE(((inmoney) table.getSelectionModel().getSelectedItem()).getIn_date().substring(0, 10)));
                number = Double.parseDouble(((inmoney) table.getSelectionModel().getSelectedItem()).getIn_price());
        ...
        s.setOnMouseClicked((event) -> {
            table.setItems(database.buildInMoneyTable("select * from inmoney where in_date between '" + search.getValue() + " 00:00:00' and '" + search.getValue() + " 23:59:59';"));
        });
    ...
    void setValues() {
        try {
            ...
            Date d = new Date();
            this.in.setIn_id(Integer.parseInt(id.getText()));
            this.in.setIn_date(((LocalDate) date.getValue()).toString() + " " + sdf.format(d));
            this.in.setIn_notes(notes.getText());
            this.in.setIn_price(price.getText());
