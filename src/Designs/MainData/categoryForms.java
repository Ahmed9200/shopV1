...
public class categoryForms {
    ...
    Label tittle = new Label("الاصناف");
    ...
    public categoryForms() {
        ...
        this.delete.setOnAction((event) -> {
            if (tools.confirmMsg("سوف يتم مسح الصنف بكل البضاعه المدرجه بداخله  من قاعده البيانات هل انت متأكد ؟")) {
                this.setValues();
                this.item.delete();
                this.clear();
            }
        ...
        table.setItems(database.buildItemsTable("select * from category"));
        TableColumn<category, Integer> idColumn = new TableColumn("الكود");
        idColumn.setMinWidth(80.0D);
        idColumn.setCellValueFactory(new PropertyValueFactory("id"));
        TableColumn<category, String> departmentColumn = new TableColumn(" اسم الصنف");
        ...
        table.setOnMouseClicked((event) -> {
            try {
                id.setText(((category) table.getSelectionModel().getSelectedItem()).getId() + "");
                name.setText(((category) table.getSelectionModel().getSelectedItem()).getName());
        ...
        s.setOnMouseClicked((event) -> {
            try {
                ...
                table.setItems(database.buildItemsTable("select * from category where id ='" + x + "';"));
            } catch (Exception var4) {
                try {
                    table.setItems(database.buildItemsTable("select * from category where name like  '%" + search.getText() + "%';"));
                } catch (Exception var3) {
    ...
    void setValues() {
        try {
            ...
            tools.InformationBox("حدث خطأ ما اثناى اضاقه الصنف");
        }
