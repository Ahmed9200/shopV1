...
public class ItemsHistory implements mainFunctions {
    ...
    public void add() {
        String strInsert = "insert into additemtemp values("
                + this.id + " , '"
                + this.name + "' , '"
                + this.barcode + "' , "
                + this.category_id + " , "
        ...
        if (isAdded) {
            tools.InformationBox("تم اضافه القطعه فى قاعده البيانات بنجاح :] ");
        } else {
    ...
    public void update() {
        String strUpdate = "update additemtemp set "
                + "name = '" + this.name + "' , "
                ...
                + "mototr_no = '" + this.motor_no + "' , "
                + "kind = '" + this.kind + "' , "
                + "size = '" + this.size + "' , "
                + "stock = '" + this.stock + "'  \n where barcode ='" + this.barcode + "';";
        boolean isUpdated = database.excuteQuery(strUpdate);
        if (isUpdated) {
            tools.InformationBox("تم تعديل القطعه فى قاعده البيانات بنجاح :] ");
        } else {
            tools.ErrorBox("هنالك خطأ ما");
        }
    ...
    public void delete() {
        String strDelete = "delete from additemtemp where barcode = '" + this.barcode + "';";
    ...
    public String getAutoNumber() {
        String strAuto = database.AutoIncrementCoulmn("additemtemp", "id");
        return strAuto;
    }
